...
public class CraftingRequirement {
	...
	public boolean testItem(ItemIdentifierStack other) {
		...
		if (use_od || use_category) {
			...
			if (nam1.equals(nam2)) return true;
			if (use_category) {
		...
		if (stack_n.getItemDamage() != other_n.getItemDamage()) if (stack_n.getHasSubtypes()) return false;
		else if (!ignore_dmg) return false;
	...
	public ArrayList<ItemIdentifier> getSubtitutes(IRequestItems target) {
		...
		for (Map.Entry<ItemIdentifier, Integer> i : avail_items.entrySet()) {
			if (this.testItem(i.getKey().makeStack(1))) if (!result.contains(i.getKey())) result.add(i.getKey());
		}
		for (ItemIdentifier i : craft_items) {
			if (this.testItem(i.makeStack(1))) if (!result.contains(i)) result.add(i);
		}
		return result;
	}
}
