...
public final class ItemIdentifier implements Comparable<ItemIdentifier> {
	...
	private static class ItemKey implements Comparable<ItemKey> {
		...
		public boolean equals(Object that) {
			if (!(that instanceof ItemKey)) return false;
		...
		public int compareTo(ItemKey o) {
			if (itemID == o.itemID) return itemDamage - o.itemDamage;
			return itemID - o.itemID;
		}
	}

	private static final ConcurrentHashMap<Integer, ItemIdentifier> _itemIdentifierIdCache = new ConcurrentHashMap<Integer, ItemIdentifier>(4096, 0.5f, 1);

	// for when things differ by NBT tags, and an itemKey isn't enough to get the full object
	private static final ConcurrentHashMap<ItemKey, ConcurrentHashMap<FinalNBTTagCompound, ItemIdentifier>> _itemIdentifierTagCache = new ConcurrentHashMap<ItemKey, ConcurrentHashMap<FinalNBTTagCompound, ItemIdentifier>>(1024, 0.5f, 1);

	private static final ConcurrentHashMap<ItemKey, ItemIdentifier> _itemIdentifierCache = new ConcurrentHashMap<ItemKey, ItemIdentifier>(4096, 0.5f, 1);
	private static final HashSet<ItemIdentifier> _badTags = new HashSet<ItemIdentifier>(128, 0.75f);

	//array of mod names, used for id -> name, 0 is unknown
	private static final ArrayList<String> _modNameList = new ArrayList<String>();
	//map of mod name -> internal LP modid, first modname gets 1
	private static final Map<String, Integer> _modNameToModIdMap = new HashMap<String, Integer>();
	//map of itemid -> modid
	...
	*/
	...
	private Map<Object, Object> getNBTBaseAsMap(NBTBase nbt) throws NoSuchFieldException, IllegalAccessException {
		if (nbt == null) {
			return null;
		}
	...
	public int compareTo(ItemIdentifier o) {
		...
		if (uniqueID < o.uniqueID) return -1;
		if (uniqueID > o.uniqueID) return 1;
		return 0;
	}
	...
	@Override
	public boolean equals(Object that) {
		if (!(that instanceof ItemIdentifier)) return false;
	...
	private static String checkNBTbadness_recurse(NBTBase nbt) throws NoSuchFieldException, IllegalAccessException {
