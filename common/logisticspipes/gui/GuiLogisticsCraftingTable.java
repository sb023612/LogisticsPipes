...
public class GuiLogisticsCraftingTable extends LogisticsBaseGuiScreen {
	...
	protected void drawGuiContainerForegroundLayer(int par1, int par2) {
		...
		if (_crafter.isFuzzy()) {
			...
			if ((mx - 35) % 18 <= 16 && (mx - 35) % 18 >= 0) if ((my - 10) % 18 <= 16 && (my - 10) % 18 >= 0) {
				...
				if (x_slot >= 0 && x_slot < 3 && y_slot >= 0 && y_slot < 3) {
					...
					} else fuzzyPanelHoverTime++;
				} else fuzzyPanelHover = -1;
			} else fuzzyPanelHover = -1;
			if (fuzzyPanelHover != -1 && fuzzyPanelHoverTime >= 60) fuzzyPanelSelection = fuzzyPanelHover;
		}
		...
		if (fuzzyPanelSelection != -1) {
			...
			fontRenderer.drawString(StringUtil.translate(PREFIX + "OreDict"), posX + 4, posY + 4, (!flag.use_od ? 0x404040 : 0xFF4040));
			fontRenderer.drawString(StringUtil.translate(PREFIX + "IgnDamage"), posX + 4, posY + 14, (!flag.ignore_dmg ? 0x404040 : 0x40FF40));
			fontRenderer.drawString(StringUtil.translate(PREFIX + "IgnNBT"), posX + 4, posY + 26, (!flag.ignore_nbt ? 0x404040 : 0x4040FF));
			fontRenderer.drawString(StringUtil.translate(PREFIX + "OrePrefix"), posX + 4, posY + 38, (!flag.use_category ? 0x404040 : 0x7F7F40));
		}
	}
