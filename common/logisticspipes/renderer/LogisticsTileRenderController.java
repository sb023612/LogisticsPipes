...
public class LogisticsTileRenderController {
		...
		}
		{
			...
			while (iter.hasNext()) {
				...
				if (data.timeout < 0 || data.isDeadEntity()) {
					data.setDead();
					if (data.sendPacket()) {
						MainProxy.sendPacketToAllWatchingChunk(this.pipe.getX(), this.pipe.getZ(), MainProxy.getDimensionForWorld(this.pipe.getWorld()),
								PacketHandler.getPacket(PowerPacketLaser.class).setColor(key).setPos(pipe.getCPipe().getLPPosition()).setRenderBall(true).setDir(ForgeDirection.UNKNOWN).setRemove(true));
					}
					iter.remove();
				}
			}
	...
	public void addLaser(ForgeDirection dir, float length, int color, boolean reverse, boolean renderBall) {
		...
		if (sendPacket) {
			MainProxy.sendPacketToAllWatchingChunk(this.pipe.getX(), this.pipe.getZ(), MainProxy.getDimensionForWorld(this.pipe.getWorld()), PacketHandler.getPacket(PowerPacketLaser.class).setColor(color).setPos(pipe.getCPipe().getLPPosition()).setRenderBall(renderBall).setDir(dir).setLength(length)
					.setReverse(reverse));
		}
	}
	...
	public void sendInit() {
		...
		while (iter.hasNext()) {
			...
			MainProxy.sendPacketToAllWatchingChunk(this.pipe.getX(), this.pipe.getZ(), MainProxy.getDimensionForWorld(this.pipe.getWorld()), PacketHandler.getPacket(PowerPacketLaser.class).setColor(key.color).setPos(pipe.getCPipe().getLPPosition()).setRenderBall(isBall).setDir(key.dir).setLength(data.length)
