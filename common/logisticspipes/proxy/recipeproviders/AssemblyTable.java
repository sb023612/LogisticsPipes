...
public class AssemblyTable implements ICraftingRecipeProvider {
	...
	public boolean importRecipe(TileEntity tile, ItemIdentifierInventory inventory) {
		if (!(tile instanceof TileAssemblyTable)) return false;
		...
		if (nextRecipe == null) nextRecipe = firstRecipe;
		if (nextRecipe == null) return false;
		...
		try {
			for (int i = 0; i < inventory.getSizeInventory() - 2; i++) {
				if (i < nextRecipe.input.length) {
					inventory.setInventorySlotContents(i, (ItemStack) nextRecipe.input[i]);
				} else {
