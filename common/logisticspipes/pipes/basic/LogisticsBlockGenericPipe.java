...
public class LogisticsBlockGenericPipe extends BlockContainer {
	...
	public void addCollisionBoxesToList(World world, int i, int j, int k, AxisAlignedBB axisalignedbb, List arraylist, Entity par7Entity) {
		...
		if(tile1 instanceof LogisticsTileGenericPipe && ((LogisticsTileGenericPipe)tile1).pipe instanceof PipeBlockRequestTable) {
		...
		if (tile1 instanceof LogisticsTileGenericPipe) {
	...
	private RaytraceResult doRayTrace(World world, int x, int y, int z, Vec3 origin, Vec3 direction) {
		...
		CoreRoutedPipe pipe = (CoreRoutedPipe) tileG.pipe;
	...
	public static void removePipe(CoreRoutedPipe pipe) {
		if (pipe == null)
			return;
	...
	public ArrayList<ItemStack> getBlockDropped(World world, int x, int y, int z, int metadata, int fortune) {
		...
		for (int i = 0; i < count; i++) {
			CoreRoutedPipe pipe = getPipe(world, x, y, z);
	...
	public void dropBlockAsItemWithChance(World world, int i, int j, int k, int l, float f, int dmg) {
		...
		for (int j1 = 0; j1 < i1; j1++) {
			...
			CoreRoutedPipe pipe = getPipe(world, i, j, k);
	...
	public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float xOffset, float yOffset, float zOffset) {
		...
		world.notifyBlocksOfNeighborChange(x, y, z, LogisticsPipes.LogisticsPipeBlock.blockID);
	...
	private boolean addOrStripGate(World world, int x, int y, int z, EntityPlayer player, CoreRoutedPipe pipe) {
	...
	private boolean addGate(EntityPlayer player, CoreRoutedPipe pipe) {
		if (!pipe.hasGate()) {
	...
	private boolean stripGate(CoreRoutedPipe pipe) {
		if (pipe.hasGate()) {
	...
	private boolean addOrStripWire(EntityPlayer player, CoreRoutedPipe pipe, IPipe.WireColor color) {
	...
	private boolean addWire(CoreRoutedPipe pipe, IPipe.WireColor color) {
	...
	private boolean stripWire(CoreRoutedPipe pipe, IPipe.WireColor color) {
		if (pipe.wireSet[color.ordinal()]) {
			if (!CoreProxy.proxy.isRenderWorld(pipe.container.worldObj)) {
				dropWire(color, pipe);
			}
	...
	private boolean addOrStripFacade(World world, int x, int y, int z, EntityPlayer player, ForgeDirection side, CoreRoutedPipe pipe) {
		RaytraceResult rayTraceResult = doRayTrace(world, x, y, z, player);
		if (player.isSneaking()) {
	...
	private boolean addFacade(EntityPlayer player, CoreRoutedPipe pipe, ForgeDirection side) {
	...
	private boolean stripFacade(CoreRoutedPipe pipe, ForgeDirection side) {
		return pipe.container.dropFacade(side);
	}

	private boolean addOrStripPlug(World world, int x, int y, int z, EntityPlayer player, ForgeDirection side, CoreRoutedPipe pipe) {
		RaytraceResult rayTraceResult = doRayTrace(world, x, y, z, player);
		if (player.isSneaking()) {
	...
	private boolean addPlug(EntityPlayer player, CoreRoutedPipe pipe, ForgeDirection side) {
		ItemStack stack = player.getCurrentEquippedItem();
	...
	private boolean stripPlug(CoreRoutedPipe pipe, ForgeDirection side) {
		return pipe.container.removeAndDropPlug(side);
	}

	private boolean stripEquipment(World world, int x, int y, int z, EntityPlayer player, CoreRoutedPipe pipe) {
	...
	public Icon getBlockTexture(IBlockAccess iblockaccess, int x, int y, int z, int side) {
		...
		if(tile instanceof LogisticsTileGenericPipe && ((LogisticsTileGenericPipe)tile).pipe instanceof PipeBlockRequestTable) {
			...
			return table.getTextureFor(side);
		}
	...
	public int isProvidingStrongPower(IBlockAccess iblockaccess, int x, int y, int z, int l) {
		CoreRoutedPipe pipe = getPipe(iblockaccess, x, y, z);
	...
	/* Registration ******************************************************** */
	public static Map<Integer, Class<? extends CoreRoutedPipe>> pipes = new HashMap<Integer, Class<? extends CoreRoutedPipe>>();
	static long lastRemovedDate = -1;
	public static Map<BlockIndex, CoreRoutedPipe> pipeRemoved = new HashMap<BlockIndex, CoreRoutedPipe>();

	public static ItemLogisticsPipe registerPipe(int key, Class<? extends CoreRoutedPipe> clas) {
	...
	public static CoreRoutedPipe createPipe(int key) {

		try {
			Class<? extends CoreRoutedPipe> pipe = pipes.get(key);
			if (pipe != null)
	...
	public static boolean placePipe(CoreRoutedPipe pipe, World world, int i, int j, int k, int blockId, int meta) {
		if (world.isRemote)
			return true;
	...
	public static CoreRoutedPipe getPipe(IBlockAccess blockAccess, int i, int j, int k) {
		...
		if(((LogisticsTileGenericPipe) tile).pipe instanceof CoreRoutedPipe) {
			return (CoreRoutedPipe) ((LogisticsTileGenericPipe) tile).pipe;
		}
		return null;
	}

	public static boolean isFullyDefined(CoreRoutedPipe pipe) {
		return pipe != null && pipe.transport != null;
	}

	public static boolean isValid(CoreRoutedPipe pipe) {
	...
	public boolean addBlockHitEffects(World worldObj, MovingObjectPosition target, EffectRenderer effectRenderer) {
		...
		CoreRoutedPipe pipe = getPipe(worldObj, x, y, z);
		if (pipe == null)
			return false;
