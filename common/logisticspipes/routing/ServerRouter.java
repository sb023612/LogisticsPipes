...
public class ServerRouter implements IRouter, Comparable<ServerRouter> {
	...
	private class UpdateRouterRunnable implements Comparable<UpdateRouterRunnable>, Runnable {
		...
		public void run() {
			...
			try {
				...
				if (p.stillNeedReplace()) return; // drop the pipe update if it still needs replace after 5 ticks.
		...
		public int compareTo(UpdateRouterRunnable o) {
			int c = 0;
			if (o.newVersion <= 0) c = newVersion - o.newVersion; // negative numbers have priority, more negative first
			if (c != 0) return 0;
	...
	public ServerRouter(UUID globalID, int dimension, int xCoord, int yCoord, int zCoord) {
		if (globalID != null) this.id = globalID;
	...
	public CoreRoutedPipe getCachedPipe() {
		if (_myPipeCache != null) return _myPipeCache.get();
		return null;
	}
		...
		if (_powerAdjacent != null) {
			if (power == null) {
				...
				for (Pair<ILogisticsPowerProvider, List<IFilter>> provider : _powerAdjacent) {
					if (!power.contains(provider)) adjacentChanged = true;
				}
			}
		}
		if (power != null) {
			if (_powerAdjacent == null) {
				...
				for (Pair<ILogisticsPowerProvider, List<IFilter>> provider : power) {
					if (!_powerAdjacent.contains(provider)) adjacentChanged = true;
				}
			}
		}
		if (_subSystemPowerAdjacent != null) {
			if (subSystemPower == null) {
				...
				for (Pair<ISubSystemPowerProvider, List<IFilter>> provider : _subSystemPowerAdjacent) {
					if (!subSystemPower.contains(provider)) adjacentChanged = true;
				}
			}
		}
		if (subSystemPower != null) {
			if (_subSystemPowerAdjacent == null) {
				...
				for (Pair<ISubSystemPowerProvider, List<IFilter>> provider : subSystemPower) {
					if (!_subSystemPowerAdjacent.contains(provider)) adjacentChanged = true;
				}
			}
		}
	...
	public void createRouteTable(int version_to_update_to, IRoutingDebugAdapter debug) {
		...
		if (_lastLSAVersion[simpleID] >= version_to_update_to && !debug.independent()) return; // this update is already done.
		...
		if (_powerAdjacent != null) powerTable = new ArrayList<Pair<ILogisticsPowerProvider, List<IFilter>>>(_powerAdjacent);
		else powerTable = new ArrayList<Pair<ILogisticsPowerProvider, List<IFilter>>>(5);
		ArrayList<Pair<ISubSystemPowerProvider, List<IFilter>>> subSystemPower;
		if (_subSystemPowerAdjacent != null) subSystemPower = new ArrayList<Pair<ISubSystemPowerProvider, List<IFilter>>>(_subSystemPowerAdjacent);
		else subSystemPower = new ArrayList<Pair<ISubSystemPowerProvider, List<IFilter>>>(5);
		...
		while ((lowestCostNode = candidatesCost.poll()) != null) {
			...
			if (lowestCostClosedFlags == null) lowestCostClosedFlags = EnumSet.noneOf(PipeRoutingConnectionType.class);
			...
			if (lsa == null) {
				...
				if (lowestCostNode.containsFlag(PipeRoutingConnectionType.canRouteTo) || lowestCostNode.containsFlag(PipeRoutingConnectionType.canRequestFrom)) routeCosts.add(lowestCostNode);
			...
			lowestCostNode.removeFlags(lowestCostClosedFlags);
			lowestCostClosedFlags.addAll(lowestCostNode.getFlags());
			if (lowestCostNode.containsFlag(PipeRoutingConnectionType.canRouteTo) || lowestCostNode.containsFlag(PipeRoutingConnectionType.canRequestFrom) || lowestCostNode.containsFlag(PipeRoutingConnectionType.canPowerSubSystemFrom)) routeCosts.add(lowestCostNode);
	...
	public void act(BitSet hasBeenProcessed, IRAction actor) {
		if (hasBeenProcessed.get(this.simpleID)) return;
		hasBeenProcessed.set(this.simpleID);
		if (!actor.isInteresting(this)) return;
	...
	public boolean hasRoute(int id, boolean active, ItemIdentifier type) {
		...
		ensureRouteTableIsUpToDate(true);
		if (getRouteTable().size() <= id) return false;
	...
	public void updateInterests() {
		if (--ticksUntillNextInventoryCheck > 0) return;
		ticksUntillNextInventoryCheck = REFRESH_TIME;
		if (iterated++ % this.simpleID == 0) ticksUntillNextInventoryCheck++; // randomly wait 1 extra tick - just so that every router doesn't tick at the same time
		if (iterated >= ServerRouter.getBiggestSimpleID()) iterated = 0;
		CoreRoutedPipe pipe = getPipe();
		if (pipe == null) return;
		if (pipe.hasGenericInterests()) this.declareGenericInterest();
		else this.removeGenericInterest();
	...
	private void removeInterest(ItemIdentifier p2) {
		...
		if (interests.isEmpty()) _globalSpecificInterests.remove(p2);
