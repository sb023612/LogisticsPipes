...
public final class DebugGuiTickHandler implements ITickHandler, Serializable, TreeExpansionListener, MouseListener {
	...
	private transient Map<Player, ServerGuiSetting> serverInfo = new HashMap<Player, ServerGuiSetting>();
	...
	private boolean isPrimitive(Class<?> clazz) {
		return clazz == Integer.class || clazz == Boolean.class || clazz == Double.class || clazz == Float.class || clazz == Long.class || clazz == UUID.class || clazz == Byte.class || clazz == String.class || clazz == ForgeDirection.class || clazz == WorldServer.class || clazz == ItemIdentifier.class
				|| clazz == ItemIdentifierStack.class;
	}
	...
	public void targetResponse(TargetMode mode, final EntityPlayer player, Object[] additions) {
		if (mode == TargetMode.None) {
			...
			if (tile == null) {
				...
				player.sendChatToPlayer(ChatMessageComponent.createFromText(ChatColor.AQUA + "Start debuging of TileEntity: " + ChatColor.BLUE + ChatColor.UNDERLINE + tile.getClass().getSimpleName() + ChatColor.AQUA + "? " + ChatColor.RESET + "<" + ChatColor.GREEN + "yes" + ChatColor.RESET + "/" + ChatColor.RED + "no"
						+ ChatColor.RESET + ">"));
				MainProxy.sendPacketToPlayer(PacketHandler.getPacket(OpenChatGui.class), (Player) player);
			}
			...
			if (entitiy == null) {
				...
				player.sendChatToPlayer(ChatMessageComponent.createFromText(ChatColor.AQUA + "Start debuging of Entity: " + ChatColor.BLUE + ChatColor.UNDERLINE + entitiy.getClass().getSimpleName() + ChatColor.AQUA + "? " + ChatColor.RESET + "<" + ChatColor.GREEN + "yes" + ChatColor.RESET + "/" + ChatColor.RED + "no"
						+ ChatColor.RESET + ">"));
				MainProxy.sendPacketToPlayer(PacketHandler.getPacket(OpenChatGui.class), (Player) player);
			}
		}
