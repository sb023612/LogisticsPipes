...
public class PipeTransportLogistics extends PipeTransport {
	...
	public ForgeDirection resolveDestination(LPTravelingItemServer data) {
		...
		} else value = getPipe().getRouteLayer().getOrientationForItem(data, blocked);
	...
	public boolean canPipeConnect(TileEntity tile, ForgeDirection side) {
		...
		if (SimpleServiceLocator.betterStorageProxy.isBetterStorageCrate(tile) || SimpleServiceLocator.factorizationProxy.isBarral(tile)
				...
				|| (this.getPipe().getUpgradeManager().hasRFPowerSupplierUpgrade() && SimpleServiceLocator.thermalExpansionProxy.isEnergyHandler(tile)) || (this.getPipe().getUpgradeManager().getIC2PowerLevel() > 0 && SimpleServiceLocator.IC2Proxy.isEnergySink(tile))) {
			return true;
		}
	...
	private boolean passToNextPipe(LPTravelingItem item, TileEntity tile) {
		...
		if (SimpleServiceLocator.buildCraftProxy.insertIntoBuildcraftPipe(tile, item)) {
			return true;
		}
		return false;
	}
	...
	private void sendItemPacket(LPTravelingItemServer item) {
		...
		MainProxy.sendPacketToAllWatchingChunk(this.container.xCoord, this.container.zCoord, MainProxy.getDimensionForWorld(this.getWorld()), (PacketHandler.getPacket(PipePositionPacket.class).setSpeed(item.getSpeed()).setPosition(item.getPosition()).setInput(item.input).setOutput(item.output)
